...
class ProbIoUloss:
    def __init__(self, eps=1e-3, mode='l1') -> None:
        self.__dict__.update(locals())
    def __call__(self, pred, target):
        gbboxes1 = ProbIoUloss.gbb_form(pred)
        gbboxes2 = ProbIoUloss.gbb_form(target)
                                     ...
                                     0], gbboxes2[:,
                                                  1], gbboxes2[:,
                                                               2], gbboxes2[:,
                                                                            3], gbboxes2[:,
                                                                                         4]
        a1, b1, c1 = ProbIoUloss.rotated_form(a1_, b1_, c1_)
        a2, b2, c2 = ProbIoUloss.rotated_form(a2_, b2_, c2_)                                                                                4]
        ...
        t3 = 0.5 * torch.log(t2 / (4 * torch.sqrt(F.relu(t3_)) + self.eps))
        B_d = (t1 / t2) + t3
        l1 = torch.sqrt(1.0 - torch.exp(-B_d) + self.eps)
        l_i = torch.pow(l1, 2.0)
        l2 = -torch.log(1.0 - l_i + self.eps)
        if(self.mode=='l1'):
            probiou = l1
        elif(self.mode=='l2'):
            probiou = l2
    ...
    @staticmethod
    def gbb_form(obb):
        xy, wh, angle = torch.split(obb,[2,2,1],axis=-1)
        return torch.concat([xy,wh.pow(2)/12.,angle],axis=-1)
    @staticmethod
