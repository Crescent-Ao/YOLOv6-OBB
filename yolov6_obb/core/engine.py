...
from yolov6_obb.models.yolo import build_model
from yolov6_obb.models.loss_obb import ComputeLoss
from yolov6_obb.models.loss_distill import ComputeLoss as ComputeLoss_distill
from yolov6_obb.utils.events import LOGGER, NCOLS, load_yaml, write_tblog, write_tbimg
from yolov6_obb.utils.ema import ModelEMA, de_parallel
from yolov6_obb.utils.checkpoint import load_state_dict, save_checkpoint, strip_optimizer
from yolov6_obb.solver.build import build_optimizer, build_lr_scheduler
from yolov6_obb.utils.RepOptimizer import extract_scales, RepVGGOptimizer
from yolov6_obb.utils.nms import xywh2xyxy
import ipdb
...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.loss_num = 4
        self.loss_info = ['Epoch', 'iou_loss', 'reg_dfl_loss', 'cls_loss','reg_angle_loss']
        if self.args.distill:
            self.loss_num += 1
    ...
    def train_before_loop(self):
        ...
        self.compute_loss = ComputeLoss(num_classes=self.data_dict['nc'],
                                        ...
                                        use_reg_dfl=self.cfg.model.head.use_reg_dfl,
                                        reg_max=self.cfg.model.head.reg_max,
                                        use_angle_dfl=self.cfg.model.head.use_angle_dfl,
    ...
    def get_data_loader(args, cfg, data_dict):
        ...
        train_loader = create_dataloader(train_path, args.img_size, args.batch_size//args.world_size,rank=args.local_rank, 
                                         data_dict=data_dict, workers=args.workers,task='train')[0]
        val_loader = None
        if args.rank in [-1, 0]:
            val_loader = create_dataloader(val_path, args.img_size, args.batch_size//args.world_size*2,rank=-1, workers=args.workers,
                                             data_dict=data_dict,task='val')[0]
