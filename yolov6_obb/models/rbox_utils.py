...
def box2corners(box):
    ...
    """
    B = box.shape[0]
    x, y, w, h, alpha = torch.split(box, [1,1,1,1,1], axis=-1)
    x4 = torch.Tensor([0.5, 0.5, -0.5, -0.5],dtype=box.dtype).reshape(1,1,4)
    x4 = x4 * w  # (B, N, 4)
    y4 = torch.Tensor(
        [-0.5, 0.5, 0.5, -0.5], dtype=box.float32).reshape((1, 1, 4))
    y4 = y4 * h  # (B, N, 4)
